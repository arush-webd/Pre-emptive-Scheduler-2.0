...
void kernel_main(void) {
    ...
    // Create a dummy "current_running" so entry.S TEST_NESTED_COUNT doesn't dereference NULL
    pcb_t* p = pcb_allocate();
    if (p) {
        p->status = PROCESS_RUNNING;
        p->nested_count = 1; // IMPORTANT: keep nonzero => irq0 takes nested_not_zero path (no preempt/stack switch)
    ...
    vga_puts("IDT/PIC/PIT initialized. Enabling interrupts...");
    sti();
