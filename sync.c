...
// Provided by scheduler.cpp as a C symbol
...
extern "C" void sem_init(sem_t *s, int initial) {
	enter_critical();
	s->value = initial;
...
extern "C" void sem_wait(sem_t *s) {
	enter_critical();
	if (s->value > 0) {
		s->value--;
		leave_critical();
...
extern "C" void sem_signal(sem_t *s) {
	enter_critical();
...
extern "C" void cond_init(cond_t *c) {
	enter_critical();
	queue_init(&c->waiters);
	leave_critical();
}

extern "C" void cond_wait(cond_t *c) {
	enter_critical();
	block_current_on(&c->waiters);
	leave_critical();
}

extern "C" void cond_signal(cond_t *c) {
	enter_critical();
	wake_one(&c->waiters);
	leave_critical();
}

extern "C" void cond_broadcast(cond_t *c) {
	enter_critical();
	wake_all(&c->waiters);
	leave_critical();
}

extern "C" void barrier_init(barrier_t *b, int total_threads) {
	enter_critical();
...
extern "C" void barrier_wait(barrier_t *b) {
	enter_critical();
	b->count++;
	if (b->count < b->total) {
